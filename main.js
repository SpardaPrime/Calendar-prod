...
;// CONCATENATED MODULE: ./src/func/other.js
...
var countHours = function countHours(h) {
  var data = h.split('-');
  var hours = [];

  for (var i = +data[0]; i <= data[1]; i += 1) {
    hours.push("".concat(i, ":00"));
  }

  return hours;
};
var tableConstructor = function tableConstructor(headers, hours) {
  var header = toConsumableArray_default()(headers.slice(1));

  var table = "\n    <table class=\"table \">\n    <tr>\n    ";
  headers.forEach(function (item) {
    table += "<th class=\"unselectable\">".concat(item, "</th>");
  });

  for (var i = 0; i < hours.length; i += 1) {
    table += "<tr data-time=\"".concat(hours[i], "\"><td  class=\"time-colum unselectable\">").concat(hours[i], "</td>");

    for (var j = 0; j < header.length; j += 1) {
      table += "<td data-day=\"".concat(header[j], "\"></td>");
    }
...
var clearEvent = function clearEvent(_ref) {
  var eventData = _ref.eventData,
      checkboxes = _ref.checkboxes,
      closeAlertBtn = _ref.closeAlertBtn,
      _ref$target = _ref.target,
      target = _ref$target === void 0 ? null : _ref$target,
      days = _ref.days,
      hoursArr = _ref.hoursArr;
  var eventWindow = document.querySelector('.event-window');
  var empty = '';

  var _days = slicedToArray_default()(days, 1),
      fistDays = _days[0];

  var _hoursArr = slicedToArray_default()(hoursArr, 1),
      fistHour = _hoursArr[0];
...
var fillTable = function fillTable(_ref2) {
  var _ref2$member = _ref2.member,
      member = _ref2$member === void 0 ? 'all' : _ref2$member,
      allEvents = _ref2.allEvents,
      users = _ref2.users;
  var table = document.querySelector('.table');
  var fillDiv = document.querySelectorAll('.div-fill-table');

  if (fillDiv) {
    fillDiv.forEach(function (item) {
  ...
  if (member === 'all' && allEvents.length) {
    allEvents.forEach(function (_ref3) {
      var title = _ref3.title,
          day = _ref3.day,
          time = _ref3.time;
      var elem = table.querySelector("[data-time=\"".concat(time, "\"]")).querySelector("[data-day=\"".concat(day, "\"]"));
      var html = "<div class=\"div-fill-table unselectable\" title=\"".concat(title, "\" data-time=\"").concat(time, "\" data-day=\"").concat(day, "\"><h5>").concat(title, "</h5></div><button class=\"btn btn-light btn-sm div-fill-table-close\">X</button>");
...
var remoteLocalStorage = function remoteLocalStorage() {
...
var enterDroppable = function enterDroppable(elem) {
  var e = elem;
  e.style.background = '#c5c6c7';
};
var leaveDroppable = function leaveDroppable(elem) {
  var e = elem;
...
/* harmony default export */ const table = (function (arr, h) {
  var headers = ['Time'].concat(toConsumableArray_default()(arr));
  var hours = countHours(h);
  var table = document.createElement('div');
  table.classList.add('parent-table');
  var html = tableConstructor(headers, hours);
...
/* harmony default export */ const head = (function (arr) {
  if (arr.length) {
...
;// CONCATENATED MODULE: ./src/modals/rewriteModal.js
/* harmony default export */ const rewriteModal = (function () {
  var modal = document.createElement('div');
...
;// CONCATENATED MODULE: ./src/mountComponent.js





/* harmony default export */ const mountComponent = (function (_ref) {
  ...
  var body = function body() {
    ...
    document.body.append(templeteNewEvent(users, days, hoursArr));
...
;// CONCATENATED MODULE: ./src/components/eventWindow/scriptNewEvent.js
...
/* harmony default export */ const scriptNewEvent = (function (_ref) {
  var createBtn = _ref.createBtn,
      closeAlertBtn = _ref.closeAlertBtn,
      ...
      hoursArr = _ref.hoursArr,
      target = _ref.target;
  var cancelWindowBtn = document.getElementById('cancel-window-btn');
  var textField = document.querySelector('.partipals-text');
  var users = [];

  var togleDisabled = function togleDisabled() {
  ...
  cancelWindowBtn.addEventListener('click', function () {
    return clearEvent({
      eventData: eventData,
  ...
  closeAlertBtn.addEventListener('click', function (e) {
    return e.target.parentNode.classList.remove('show');
  });
  eventTitle.addEventListener('input', togleDisabled);
  document.addEventListener('change', function (e) {
    var targ = e.target.closest('.check-event-users');
    if (!targ) return;

    if (targ.checked) {
      users.push(targ.value);
    } else {
      var idx = users.findIndex(function (item) {
        return item === targ.value;
      });
      users = [].concat(toConsumableArray_default()(users.slice(0, idx)), toConsumableArray_default()(users.slice(idx + 1)));
    }
...
;// CONCATENATED MODULE: ./src/modals/consfigWindow.js


/* harmony default export */ const consfigWindow = (function () {
  ...
  var addPersBtn = document.getElementById('conf-add-person');
  var createCalBtn = document.getElementById('create-cal-con');
  addPersBtn.addEventListener('click', function () {
    var elements = document.querySelectorAll('.person-name');
    var div = document.createElement('div');
  ...
  document.addEventListener('change', function (e) {
    var targ = e.target.closest('.conf-name');
    if (!targ) return;
    var res = false;
    var elems = document.querySelectorAll('.conf-name');
...
var users = JSON.parse(localStorage.getItem('dataCalUsers')) || [];
var days = JSON.parse(localStorage.getItem('dataCalDays')) || [];
var hours = JSON.parse(localStorage.getItem('dataCalHours')) || '';
var member = 'all';
var allEvents = JSON.parse(localStorage.getItem('dataCalAllEvents')) || [];
var hoursArr = hours && countHours(hours);
consfigWindow();
var confWind = {
...
var start = function start() {
  ...
  var checkboxes = document.querySelectorAll('.check-event-users');
  var eventData = {
  ...
  buttons.rewriteBtn.addEventListener('click', function () {
    rewriteModal.modal.style.display = 'flex';
  });
  rewriteModal.noBtn.addEventListener('click', function () {
    rewriteModal.modal.style.display = 'none';
  });
  rewriteModal.yesBtn.addEventListener('click', function () {
    rewriteModal.modal.style.display = 'none';
    remoteLocalStorage();
    document.location.reload();
  });
  buttons.createBtn.addEventListener('click', function (e) {
    var event = {
      title: eventData.titleEvent.value,
      day: eventData.eventDay.value,
      time: eventData.eventTime.value
    };
    var reply = allEvents.find(function (_ref) {
      var day = _ref.day,
          time = _ref.time;
      return day === event.day && time === event.time;
    });
    ...
    var usersOnEvent = eventData.participants.textContent.split('; ');
    usersOnEvent.forEach(function (item) {
      var idx = users.findIndex(function (_ref2) {
        var name = _ref2.name;
        return name === item;
      });
  ...
  var delElement = {};
  document.addEventListener('click', function (e) {
    var targ = e.target.closest('.div-fill-table-close');
    if (!targ) return;
    var textDiv = targ.previousElementSibling;
    var parentDiv = targ.parentElement;
    var time = textDiv.dataset.time;
    var day = textDiv.dataset.day;
    delElement = {
      textDiv: textDiv,
      parentDiv: parentDiv,
      time: time,
    ...
    var text = "<".concat(textDiv.textContent, ">");
    var textForDelModal = "Are you sure you want to delete ".concat(text, " event ?");
    delModal.modal.style.display = 'flex';
    delModal.text.textContent = textForDelModal;
  });
  delModal.noBtn.addEventListener('click', function () {
    delModal.modal.style.display = 'none';
  });
  delModal.yesBtn.addEventListener('click', function () {
    var _delElement = delElement,
        textDiv = _delElement.textDiv,
        parentDiv = _delElement.parentDiv,
        time = _delElement.time,
        day = _delElement.day,
        targ = _delElement.targ;
    var idx = allEvents.findIndex(function (item) {
      return item.time === time && item.day === day;
    });
    allEvents = [].concat(toConsumableArray_default()(allEvents.slice(0, idx)), toConsumableArray_default()(allEvents.slice(idx + 1)));
    users.forEach(function (item, i) {
      var idxU = item.events.findIndex(function (items) {
        return items.time === time && items.day === day;
      });

      if (idxU !== -1) {
        users[i].events = [].concat(toConsumableArray_default()(users[i].events.slice(0, idxU)), toConsumableArray_default()(users[i].events.slice(idxU + 1)));
      }
    });
    textDiv.remove();
    targ.remove();
  ...
  document.addEventListener('mousedown', function (event) {
    var targ = event.target.closest('.div-fill-table');
    if (!targ) return;
    var shiftX = event.clientX - targ.getBoundingClientRect().left;
    var shiftY = event.clientY - targ.getBoundingClientRect().top;
    var replaceField;
    targ.style.position = 'absolute';
    targ.style.zIndex = 1000;
    targ.style.height = "".concat(targ.offsetHeight, "px");
    targ.style.width = "".concat(targ.offsetWidth, "px");
    ...
    var moveAt = function moveAt(pageX, pageY) {
      targ.style.left = "".concat(pageX - shiftX, "px");
      targ.style.top = "".concat(pageY - shiftY, "px");
    };
    ...
    function onMouseMove(e) {
      ...
      targ.hidden = false;
      if (!elemBelow) return;
      var droppableBelow = elemBelow.closest('td[data-day]');
    ...
    targ.addEventListener('mouseup', function () {
      var day = targ.dataset.day;
      var time = targ.dataset.time;
      var title = targ.title;
      var changedDay = replaceField.dataset.day;
      var changedTime = replaceField.parentElement.dataset.time;
      replaceField.style.background = 'white';
      var idx = allEvents.findIndex(function (item) {
        return item.day === day && item.time === time;
      });
      allEvents = [].concat(toConsumableArray_default()(allEvents.slice(0, idx)), [{
        title: title,
        day: changedDay,
        time: changedTime
      }], toConsumableArray_default()(allEvents.slice(idx + 1)));
      users.forEach(function (_ref3, i) {
        var events = _ref3.events;
        var idxU = events.findIndex(function (item) {
          return item.day === day && item.time === time;
        });

        if (idxU !== -1) {
          users[i].events = [].concat(toConsumableArray_default()(users[i].events.slice(0, idxU)), [{
            title: title,
            day: changedDay,
            time: changedTime
          }], toConsumableArray_default()(users[i].events.slice(idxU + 1)));
        }
      });
      saveLocalStorage({
        users: users,
...
if (!users.length) {
  confWind.createBtnCal.addEventListener('click', function () {
    var inpNames = document.querySelectorAll('.conf-name');
    var timeFrom = document.getElementById('t-from');
    var timeTo = document.getElementById('t-to');
    var daysFromTo = document.getElementById('d-from-to');
    var namesUsers = [];
    inpNames.forEach(function (item) {
      if (item.value) {
        namesUsers.push({
          name: item.value,
          events: []
    ...
    users = [].concat(namesUsers);
    days = toConsumableArray_default()(daysFromTo.value.split(','));
    hours = "".concat(timeFrom.value, "-").concat(timeTo.value);
    member = 'all';
    allEvents = [];
